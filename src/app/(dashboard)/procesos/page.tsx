...
import { BotonActualizarRama } from "@/components/boton-actualizar-rama";
import { errorMessage } from "@/lib/api";
import { getUser } from "@/lib/auth";
import { venceUI } from "@/lib/vencimiento";
...
// Orden por urgencia (nivel 3, dentro de un tipo): abiertos con fecha (más
// próximo/vencido primero) → abiertos sin fecha → cerrados/archivados.
const grupoUrgencia = (i: ProcesoListItem) =>
  i.estado === "CERRADO" || i.estado === "ARCHIVADO" ? 2 : i.fechaLimite ? 0 : 1;

type Vista = "jurisdiccion" | "todos" | "mios";

/** Toggle de vista (arriba a la derecha): Jurisdicción · Todos · Míos. */
function ToggleVistas({ vista }: { vista: Vista }) {
  const tab = (href: string, key: Vista, label: string) => (
    <Link
      href={href}
      className={`rounded-md px-3 py-1.5 font-medium ${vista === key ? "bg-indigo-600 text-white" : "text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white"}`}
    >
      {label}
    </Link>
  );
  return (
    <div className="inline-flex rounded-lg border border-slate-200 p-0.5 text-sm dark:border-slate-700">
      {tab("/procesos", "jurisdiccion", "Jurisdicción")}
      {tab("/procesos?vista=todos", "todos", "Todos")}
      {tab("/procesos?vista=mios", "mios", "Míos")}
    </div>
  );
}

/** Tabla de procesos (compartida por el nivel 3 del catálogo y las vistas planas
 *  "Todos"/"Míos"). Cada fila navega a la ficha del proceso. */
function TablaProcesos({
  items,
  nombreArea,
}: {
  items: ProcesoListItem[];
  nombreArea: (slug: string | null) => string;
}) {
            ...
            return (
              ...
              >
                ...
                <td className={`px-5 py-3 align-top text-xs ${v.cls}`}>
                  {v.fecha}
                  {t.fechaLimite && t.plazoDias != null && (
                    <div className="text-[11px] font-normal text-slate-400">
                      {t.plazoDias} {t.plazoTipoDias === "calendario" ? "días calendario" : "días hábiles"}
                    </div>
                  )}
...
/** Vistas planas "Todos" (todos los procesos) y "Míos" (donde soy el responsable),
 *  ordenadas vencidos→por vencer→al día→sin fecha→cerrados (orden server-side, estable
 *  a través de la paginación). Es la vista del inicio, pero completa y navegable. */
function ListaProcesosPlana({ mios }: { mios: boolean }) {
  const u = getUser();
  const uid = u?.id ?? "";
  ...
  useEffect(() => {
    ...
    listProcesos({
      ...
      estado,
      q: q || undefined,
      conNovedades: conNovedades || undefined,
      page,
    })
      .then((r) => {
        setItems(r.items);
    ...
    <RolEmpresaGuard roles={["JURIDICO"]}>
      <div>
        <PageHeader
          ...
          action={
            <div className="flex items-center gap-2">
              ...
              {puedeEditar && <NuevoProcesoMenu href="/procesos/nuevo" />}
            </div>
          }
        />
        ...
        {loading ? (
          ...
          <EmptyState
            title={mios ? "No tienes procesos asignados" : "No hay procesos"}
            description={mios ? "Cuando seas el responsable de un proceso, aparecerá aquí con su vencimiento." : "Aún no hay procesos en el despacho."}
          />
        ) : (
          <>
            <TablaProcesos items={items} nombreArea={nombreArea} />
            {total > pageSize && (
              <div className="mt-3 flex items-center justify-between text-sm text-muted">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="ghost" onClick={() => setPage((p) => p + 1)} disabled={!hayMas}>
                    Siguiente →
                  </Button>
                </div>
              </div>
...
/** Navegación del catálogo por jurisdicción → categoría → tipo → procesos (nivel 3). */
function CatalogoProcesos() {
  ...
  // Tipos de la jurisdicción abierta (nivel 1).
  const tiposDeNivel1 = useMemo(
    () => tipos.filter((t) => t.jurisdiccion === jurSel),
    [tipos, jurSel],
  );
  ...
  // ── NIVEL 1: tarjetas de jurisdicción ──
  if (!jurSel) {
    const tarjetas = JURISDICCIONES.map((j) => ({
      key: j as string,
      label: sinJurisdiccion(JURISDICCION_LABEL[j]),
      n: conteoTiposPorJur[j] ?? 0,
      href: `/procesos?jurisdiccion=${j}`,
    }));
    return (
      <RolEmpresaGuard roles={["JURIDICO"]}>
        <div>
          <PageHeader
            title="Procesos"
            subtitle="Elige una jurisdicción para ver sus procesos."
            action={
              <div className="flex items-center gap-2">
                <ToggleVistas vista="jurisdiccion" />
                {accionNuevo}
              </div>
            }
          />
  ...
  // Etiqueta y enlaces del nodo de nivel 1 abierto (jurisdicción).
  const nivel1Label = JURISDICCION_LABEL[jurSel as Jurisdiccion]
    ? sinJurisdiccion(JURISDICCION_LABEL[jurSel as Jurisdiccion])
    : "Procesos";
  // El "volver" desde el nivel 3 regresa a la categoría civil cuando hay una (cat).
  const hrefNivel2 = catSel
    ? `/procesos?jurisdiccion=${jurSel}&cat=${catSel}`
    : `/procesos?jurisdiccion=${jurSel}`;
  const hrefTipo = (id: string) =>
    catSel
      ? `/procesos?jurisdiccion=${jurSel}&cat=${catSel}&tipo=${id}`
      : `/procesos?jurisdiccion=${jurSel}&tipo=${id}`;
  ...
  const categoriasJur = jurSel ? categorias.filter((c) => c.jurisdiccion === jurSel) : [];
    ...
    if (usaCategorias && catObjActual) {
      ...
      return (
        <RolEmpresaGuard roles={["JURIDICO"]}>
          <div>
            <Link
              href={`/procesos?jurisdiccion=${jurSel}`}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {cards.map((c) => {
                ...
                return vacia ? (
                  ...
                  <Link
                    key={c.slug}
                    href={`/procesos?jurisdiccion=${jurSel}&cat=${c.slug}`}
  ...
  return (
    <RolEmpresaGuard roles={["JURIDICO"]}>
      ...
      {loading ? (
        ...
        <TablaProcesos items={ordenados} nombreArea={nombreArea} />
      )}
      </div>
    </RolEmpresaGuard>
  );
...
function ProcesosInner() {
  ...
  if (vistaParam === "todos") return <ListaProcesosPlana mios={false} />;
  if (vistaParam === "mios") return <ListaProcesosPlana mios />;
  return <CatalogoProcesos />;
}
